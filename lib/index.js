...
class Service extends AdapterService {
  ...
  _patch (id, data, params = {}) {
    ...
    if (id !== null) {
      ...
      query = Object.assign({}, params, { query: idQuery });
    }

    const getOrFindResult = this._getOrFind(id, query).then(result => {
      if (!Array.isArray(result)) {
        return result;
      }

      return result.map(item => item[this.id]);
    });

    let seqOptions;
    return getOrFindResult.then(instanceOrIds => {
      if (!Array.isArray(instanceOrIds)) {
        seqOptions = Object.assign({}, params.sequelize, { raw: false });

        return instanceOrIds.update(data, seqOptions)
          .then(() => {
      ...
      const findQuery = Object.assign(
        { [this.id]: { $in: instanceOrIds } },
        this.filterQuery(params).filters
      );

      const findParams = Object.assign({}, params, { query: findQuery });

      seqOptions = Object.assign(
        { raw: this.raw },
        params.sequelize,
        { where: { [this.id]: { [this.Op.in]: instanceOrIds } } }
      );
